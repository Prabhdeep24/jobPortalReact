...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://job-portal-k4y1crk42-prabhdeep24s-projects.vercel.app/api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`https://job-portal-k4y1crk42-prabhdeep24s-projects.vercel.app/api/v1/job/${params.id}`);
        if (res.data.success) {
